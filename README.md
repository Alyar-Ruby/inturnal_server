...
This is a server-side app for Inturnal built in Node.js using ES6 and Express with Code Coverage and JWT Authentication. Follows [Airbnb's Javascript style guide](https://github.com/airbnb/javascript).
...
Clone the repo:
```sh
git clone git@github.com:RipBullNetworks/inturnal-server.git
cd inturnal-server
```
...
DEBUG=inturnal-server:* yarn start
```
...
```sh
# Install heroku cli and login to your heroku account
1. heroku login

# set heroku remote (done only once)
2. heroku git:remote -a inturnal-server
